...
namespace Oppilaitos
{
    ...
    public partial class avainHenkiloFM : Form
    {
        DataTable oppilaitos = new DataTable();
        DataTable vastuuHenkilot = new DataTable();
        DataTable yhteys = new DataTable();
        public avainHenkiloFM()
        {
            InitializeComponent();
        }

        private void avainHenkiloFM_Load(object sender, EventArgs e)
        {
            taytaOppilaitosTaulukko();
            taytaVastuuHenkilotTaulukko();
            valitseOppilaitosCB.DataSource = oppilaitos;
            valitseOppilaitosCB.DisplayMember = "Onimi";
            
        }

        private void valitseOppilaitosCB_SelectedIndexChanged(object sender, EventArgs e)
        {
            string viite = oppilaitos.Rows[valitseOppilaitosCB.SelectedIndex]["OID"].ToString();

            oppilaitosOLB.Text = oppilaitos.Rows[valitseOppilaitosCB.SelectedIndex]["OKatuosoite"].ToString();
            opppilaitosPnLB.Text = oppilaitos.Rows[valitseOppilaitosCB.SelectedIndex]["OPostinumero"].ToString();
            oppilaitosKaLB.Text = oppilaitos.Rows[valitseOppilaitosCB.SelectedIndex]["OPostitoimipaikka"].ToString();
            oppilaitosPuhLB.Text = oppilaitos.Rows[valitseOppilaitosCB.SelectedIndex]["OPuhelin"].ToString();
            yhteys = vastuuHenkilot.Select("OID =" + viite).CopyToDataTable();
            valitseVastuuHenkiloCB.DataSource = yhteys;
            valitseVastuuHenkiloCB.DisplayMember = "VNimi";
        }

        private void valitseVastuuHenkiloCB_TextChanged(object sender, EventArgs e)
        {
            vastuuTitteliLB.Text = yhteys.Rows[valitseVastuuHenkiloCB.SelectedIndex]["VTitteli"].ToString();
            vastuuKpLB.Text = yhteys.Rows[valitseVastuuHenkiloCB.SelectedIndex]["VSijainti"].ToString();
            vastuuEmailLB.Text = yhteys.Rows[valitseVastuuHenkiloCB.SelectedIndex]["VSahkoposti"].ToString();
            vastuuPuhLB.Text = yhteys.Rows[valitseVastuuHenkiloCB.SelectedIndex]["VPuhelin"].ToString();
        }
